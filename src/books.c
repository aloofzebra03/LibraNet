...
void rent_book(const char *isbn, char *response) {
    ...
    if (found) {
        if (book.copies > 0) {
            ...
            sleep(15);  // Simulate a delay
            fwrite(&book, sizeof(Book), 1, file);
            snprintf(response, BUFFER_SIZE, "Book has been rented successfully!\n");
        } else {
            snprintf(response, BUFFER_SIZE, "No copies available for rent.\n");
        }
    } else {
        snprintf(response, BUFFER_SIZE, "Book couldn't be found.\n");
    }
...
void return_book(const char *isbn, char *response) {
    pthread_mutex_lock(&mutex);  // Lock the mutex
